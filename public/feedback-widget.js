...
(function () {
  ...
  const STORAGE_KEY = 'fv_annotations_v2';
  ...
  panel.innerHTML = `
    ...
    <div class="fba-pf">
      <button class="fba-clear" id="fba-clear">清空全部</button>
  ...
  document.getElementById('fba-clear').addEventListener('click', (e) => {
    e.stopPropagation();
    if (!confirm('确定清空所有标注？')) return;
    annotations = []; persist(); clearPins(); renderList();
  });
  ...
  document.getElementById('fba-export').addEventListener('click', async (e) => {
    ...
    let saved = false;
    try {
      const token = localStorage.getItem('fluxvita_token') || localStorage.getItem('jowork_token') || '';
