// apps/fluxvita — chat route with premium dispatcher support
...
import { dispatch } from '@jowork/premium';
...
export function chatRouter(): Router {
  ...
  router.post('/api/sessions/:id/messages', authenticate, async (req, res, next) => {
    try {
      ...
      const systemPrompt = agent?.system_prompt ?? 'You are FluxVita, a helpful AI coworker that knows your business.';

      // Premium dispatcher auto-selects claude-agent engine when premium is active
