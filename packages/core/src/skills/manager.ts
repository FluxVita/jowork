...
import { getOrCreateBridge, type McpServerConfig } from '../agent/mcp-bridge.js';
...
export function listSkills(): SkillRecord[] {
  ensureSkillsTable();
  const db = getDb();
  const rows = db.prepare('SELECT * FROM skills ORDER BY installed_at').all() as Record<string, unknown>[];
...
export function getActiveSkills(): SkillRecord[] {
  ensureSkillsTable();
  const db = getDb();
  const rows = db.prepare('SELECT * FROM skills WHERE is_active = 1').all() as Record<string, unknown>[];
  return rows.map(rowToRecord).map(parseSkillRecord);
}
...
export function installSkill(manifest: SkillManifest): SkillRecord {
  ensureSkillsTable();
  const db = getDb();

  // 检查是否已安装
  const existing = db.prepare('SELECT id FROM skills WHERE id = ?').get(manifest.id) as { id: string } | undefined;
  if (existing) {
    // 更新
...
export function setSkillActive(id: string, active: boolean): boolean {
  ensureSkillsTable();
  const db = getDb();
...
/** 获取所有活跃 Skill 的工具定义 */
export function getAllSkillToolDefs(): AnthropicToolDef[] {
...
export async function startSkillMcpServers(manifest: SkillManifest): Promise<void> {
  if (!manifest.mcp_servers || manifest.mcp_servers.length === 0) return;
...
function rowToRecord(row: Record<string, unknown>): SkillRecord {
  return {
    ...
    manifest_json: row['manifest_json'] as string,
