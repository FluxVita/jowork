...
export function getUserSetting(userId: string, key: string): string | null {
  ...
  const row = db.prepare('SELECT value_encrypted FROM user_settings WHERE user_id = ? AND key = ?')
    .get(userId, key) as { value_encrypted: string } | undefined;
...
/** 列出用户所有配置（键名列表，不含值） */
export function listUserSettingKeys(userId: string): { key: string; updated_at: string }[] {
  ensureTable();
  const db = getDb();
  return db.prepare('SELECT key, updated_at FROM user_settings WHERE user_id = ? ORDER BY key')
    .all(userId) as { key: string; updated_at: string }[];
}

/** 删除用户配置 */
export function deleteUserSetting(userId: string, key: string) {
  ensureTable();
  const db = getDb();
...
export function getOrgSetting(key: string): string | null {
  ensureScopedTable();
  const db = getDb();
  const row = db.prepare(
    "SELECT value_enc FROM scoped_settings WHERE scope = 'org' AND scope_id = 'default' AND key = ?"
  ).get(key) as { value_enc: string } | undefined;
  if (!row) return null;
  try { return decrypt(row.value_enc); } catch { return null; }
}
...
export function getScopedValue(
  ...
  userId: string,
  groupIds: string[] = [],
): { value: string; source: 'user' | 'group' | 'org' } | null {
  ensureScopedTable();
  const db = getDb();

  // 1. 用户级
  const userRow = db.prepare(
    "SELECT value_enc FROM scoped_settings WHERE scope = 'user' AND scope_id = ? AND key = ?"
  ).get(userId, key) as { value_enc: string } | undefined;
  if (userRow) {
    try { return { value: decrypt(userRow.value_enc), source: 'user' }; } catch { /* skip */ }
  }

  // 2. 群组级（按 groupIds 顺序取第一个匹配）
  for (const gid of groupIds) {
    const groupRow = db.prepare(
      "SELECT value_enc FROM scoped_settings WHERE scope = 'group' AND scope_id = ? AND key = ?"
    ).get(gid, key) as { value_enc: string } | undefined;
    if (groupRow) {
      try { return { value: decrypt(groupRow.value_enc), source: 'group' }; } catch { continue; }
    }
  }

  // 3. org 级
  const orgVal = getOrgSetting(key);
  if (orgVal !== null) return { value: orgVal, source: 'org' };
