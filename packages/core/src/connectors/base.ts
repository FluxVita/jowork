...
/** 加密缓存写入 */
export function cacheSet(uri: string, content: string, contentType: string, ttlSeconds: number) {
...
/** 加密缓存读取 */
export function cacheGet(uri: string): { content: string; content_type: string } | null {
  const key = createHash('sha256').update(uri).digest('hex').slice(0, 16);
  const filePath = join(config.cache_dir, `${key}.enc`);
...
/** 使缓存失效 */
export function cacheInvalidate(uri: string) {
  const key = createHash('sha256').update(uri).digest('hex').slice(0, 16);
  const filePath = join(config.cache_dir, `${key}.enc`);
  try { unlinkSync(filePath); } catch { /* ignore */ }
}
...
export function cacheCleanup() {
  if (!existsSync(config.cache_dir)) return;
  ...
  if (cleaned > 0) log.info(`Cache cleanup: removed ${cleaned} expired entries`);
}
