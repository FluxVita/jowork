...
function getClient() {
    ...
    const OSSClass = require('ali-oss');
...
export class AliyunOSSConnector {
    ...
    async fetch(uri, _userContext) {
        ...
        if (!parsed)
            throw new Error(`Invalid OSS URI: ${uri}`);
    ...
    async fetchDateSessions(uid, sessionType, date) {
    ...
    async fetchSessionFile(ossPath) {
        const client = getClient();
        const buf = await client.get(ossPath);
        const json = JSON.parse(buf.content.toString('utf-8'));
...
function formatSession(json, path) {
    ...
    md += `- **入口**: ${json.entrance}\n`;
    if (json.stop_reason)
        md += `- **结束原因**: ${json.stop_reason}\n`;
    md += '\n';
    if (!json.model_loops?.length)
        return md + '（无对话内容）';
