...
/** 截断单条内容到 token 上限 */
function truncateContent(text, maxTokens) {
    const maxChars = maxTokens * 2;
    if (text.length <= maxChars)
        return text;
    return text.slice(0, maxChars) + '\n...(内容过长，已截断)';
}
function getArchivePepOpts(userId) {
    let user = null;
    try {
...
export function buildArchiveTextForSummary(messages, userId) {
 ...
 */
export async function maybeArchiveAndSummarize(sessionId, userId) {
    const messages = getMessages(sessionId);
    if (messages.length < ARCHIVE_MESSAGE_THRESHOLD) {
        // 检查 token 量
        const totalTokens = messages.reduce((sum, m) => sum + estimateTokens(m.content), 0);
        if (totalTokens < ARCHIVE_TOKEN_THRESHOLD)
            return;
    }
 ...
 */
export function buildContextWindow(messages, sessionSummary) {
    // 从最新消息向前扫描，估算 token
    let totalTokens = 0;
    let cutoffIdx = 0;
    ...
    for (const msg of windowMessages) {
        if (msg.role === 'user') {
            ...
            let parsedInput = {};
            try {
                parsedInput = JSON.parse(msg.content);
            }
            catch { /* empty */ }
...
function pushOrMerge(result, role, blocks) {
