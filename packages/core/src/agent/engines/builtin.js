...
import { routeModel, streamModelWithTools, recordModelCost, getKlaudeInfo, } from '../../models/router.js';
...
import { getToolDefinitions, getBuiltinTool, initTools } from '../tools/registry.js';
...
import { sanitizeToolResult, sanitizeOutput, getSecurityPromptSegment, } from '../../policy/context-pep.js';
...
async function* callModelStreamWithRetry(params, maxRetries = MODEL_RETRY_MAX) {
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (attempt === maxRetries || is4xxError(err) || isConnectionError(err))
                throw err;
...
function getToolProgressMessage(name, input) {
    switch (name) {
        case 'search_data': {
            const q = input['query']?.slice(0, 40);
        ...
        case 'fetch_content': {
            const uri = input['uri']?.slice(0, 60);
            const fresh = input['fresh'] === true;
            if (uri)
                return fresh ? `正在拉取最新版: ${uri}` : `正在读取: ${uri}`;
            return '正在读取内容...';
        }
        case 'run_query': {
            const src = input['source'];
        ...
        case 'write_memory': {
            const t = input['title']?.slice(0, 40);
        ...
        case 'query_posthog': {
            ...
            const distinct = input['distinct_id']?.slice(0, 30)
                ?? input['person_id']?.slice(0, 30);
            const actionLabel = {
                get_person: '查询用户信息',
                get_events: '拉取事件列表',
        ...
        case 'query_oss_sessions': {
            const action = input['action'];
            const uid = input['uid']?.slice(0, 20);
            const actionLabel = {
        ...
        case 'create_gitlab_mr': {
            ...
            const branch = input['branch_name'] ?? input['source_branch'];
        ...
        case 'lark_send_message': {
            const receiveId = input['receive_id']?.slice(0, 30);
            return receiveId ? `正在发送飞书消息至 ${receiveId}...` : '正在发送飞书消息...';
        }
        case 'lark_create_calendar_event': {
            const summary = input['summary']?.slice(0, 30);
...
export class BuiltinEngine {
    ...
    async *run(opts) {
        ...
        const pepOpts = {
            userId,
            role,
            channelType: (opts.channel?.type ?? 'web'),
            isGroupChat,
        };
        // Session 管理
        ...
        // 工具上下文
        const toolCtx = { user_id: userId, role, session_id: sessionId };
        let toolDefs = getToolDefinitions();
        const user = getUserById(userId);
        if (user && user.role !== 'owner') {
            const userServices = resolveServicesForUser(user);
            const allowedToolNames = new Set(userServices.filter(s => s.type === 'tool').map(s => s.config['tool_name']).filter(Boolean));
...
/** 用模型生成简短会话标题 */
async function generateSessionTitle(sessionId, userMessage, assistantReply, userId) {
    try {
