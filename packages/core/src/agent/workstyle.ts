...
/** 读取用户的工作方式文档 */
export function getWorkstyle(userId: string): string | null {
...
/** 保存用户的工作方式文档 */
export function saveWorkstyle(userId: string, content: string): void {
  ensureDir();
  writeFileSync(getPath(userId), content, 'utf-8');
}

/** 获取工作方式的 prompt 片段（为空则返回空字符串） */
export function getWorkstylePrompt(userId: string): string {
