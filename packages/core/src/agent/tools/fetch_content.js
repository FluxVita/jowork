...
export const fetchContentTool = {
    ...
    async execute(input, ctx) {
        ...
        const obj = getObjectByUri(uri);
        if (!obj)
            return `ERROR: 未找到 URI 为 "${uri}" 的数据对象`;
        // 权限检查
        const access = checkAccess(user, obj, 'read');
        if (!access.allowed)
            return 'ERROR: 无权访问此数据对象';
        ...
        if (!connector)
            return `ERROR: 数据源 ${obj.source} 的连接器不可用`;
        try {
...
function formatWithRange(fullContent, label, storageType, offset, limit) {
...
/** 统一格式化分段读取结果 */
function formatRangeResult(content, totalLength, hasMore, label, storageType, offset, limit) {
...
//# sourceMappingURL=fetch_content.js.map