/**
 * lark/send_message.ts
 * 内置飞书工具：发送消息到群聊或用户
 *
 * 使用用户自己的 user_access_token，以用户身份发送
 */
import type { Tool, ToolContext } from '../../types.js';
import { getLarkUserToken, TOKEN_MISSING_MSG, larkApiWithUserToken } from './auth.js';

export const larkSendMessageTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, ctx: ToolContext): Promise<string> {
    ...
    const receiveId = (input['receive_id'] as string)?.trim();
    const receiveIdType = (input['receive_id_type'] as string) ?? 'chat_id';
    const content = (input['content'] as string)?.trim();
    const msgType = (input['msg_type'] as string) ?? 'text';

    if (!receiveId || !content) return 'ERROR: receive_id 和 content 不能为空';
    ...
    try {
      const resp = await larkApiWithUserToken<{ message_id?: string }>(
        userToken,
        '/im/v1/messages',
        {
          method: 'POST',
