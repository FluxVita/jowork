...
export function getLarkUserToken(userId: string): string | null {
...
/** 带用户 token 调用飞书 API */
export async function larkApiWithUserToken<T = unknown>(
  userToken: string,
  path: string,
  ...
  return resp.json() as Promise<{ code: number; msg: string; data: T }>;
}
