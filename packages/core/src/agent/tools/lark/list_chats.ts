...
export const larkListChatsTool: Tool = {
  ...
  async execute(input: Record<string, unknown>, ctx: ToolContext): Promise<string> {
    ...
    try {
      const resp = await larkApiWithUserToken<{ items?: ChatItem[]; has_more?: boolean }>(
        userToken,
        '/im/v1/chats',
        { params: { page_size: '50' } },
      );

      if (resp.code !== 0) return `获取群列表失败（code=${resp.code}）：${resp.msg}`;
      ...
      if (chats.length === 0) return query ? `未找到名称包含「${query}」的群聊。` : '暂无群聊数据。';

      return `共 ${chats.length} 个群聊：\n` + chats.map(c =>
        `- ${c.name}（chat_id: ${c.chat_id}${c.member_count ? '，成员数: ' + c.member_count : ''}）`
      ).join('\n');
    } catch (err) {
  ...
  async executeStructured(input: Record<string, unknown>, ctx: ToolContext): Promise<{ text: string; structured: StructuredResult }> {
    const userToken = getLarkUserToken(ctx.user_id);
    if (!userToken) return { text: TOKEN_MISSING_MSG, structured: { type: 'list', items: [], total: 0 } };

    const query = (input['query'] as string | undefined)?.toLowerCase();
    const limit = Math.min((input['limit'] as number) ?? 20, 50);

    try {
      const resp = await larkApiWithUserToken<{ items?: ChatItem[] }>(
        userToken,
        '/im/v1/chats',
        { params: { page_size: '50' } },
      );

      if (resp.code !== 0) return { text: `获取群列表失败：${resp.msg}`, structured: { type: 'list', items: [], total: 0 } };

      let chats = resp.data?.items ?? [];
      if (query) chats = chats.filter(c => c.name?.toLowerCase().includes(query));
      chats = chats.slice(0, limit);

      const items: StructuredListItem[] = chats.map(c => ({
        title: c.name,
        meta: c.chat_id,
