...
function parseLocalTimestamp(dateStr) {
    ...
    if (isNaN(d.getTime()))
        throw new Error(`无法解析时间：${dateStr}`);
    return String(Math.floor(d.getTime() / 1000));
}
export const larkCreateCalendarEventTool = {
    ...
    async execute(input, ctx) {
        ...
        const startTimeStr = input['start_time']?.trim();
        const endTimeStr = input['end_time']?.trim();
        const description = input['description']?.trim();
        const location = input['location']?.trim();
        const attendeeOpenIds = input['attendee_open_ids'];
        ...
        try {
            // 获取主日历 ID
            const calListResp = await larkApiWithUserToken(userToken, '/calendar/v4/calendars');
            if (calListResp.code !== 0)
                return `获取日历失败（code=${calListResp.code}）：${calListResp.msg}`;
            ...
            if (location)
                eventBody['location'] = { name: location };
            if (attendeeOpenIds && attendeeOpenIds.length > 0) {
                eventBody['attendee_ability'] = 'none';
            }
            const createResp = await larkApiWithUserToken(userToken, `/calendar/v4/calendars/${primaryCal.calendar_id}/events`, { method: 'POST', body: eventBody });
            if (createResp.code !== 0)
                return `创建事件失败（code=${createResp.code}）：${createResp.msg}`;
            const eventId = createResp.data?.event?.event_id;
            // 如果有参与人，追加邀请
            if (attendeeOpenIds && attendeeOpenIds.length > 0 && eventId) {
                try {
                    await larkApiWithUserToken(userToken, `/calendar/v4/calendars/${primaryCal.calendar_id}/events/${eventId}/attendees`, {
                        method: 'POST',
                        body: {
...
//# sourceMappingURL=create_calendar_event.js.map