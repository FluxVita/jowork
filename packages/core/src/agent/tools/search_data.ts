import type { Tool, ToolContext, StructuredListItem, StructuredResult } from '../types.js';
...
async function doSearch(input: Record<string, unknown>, ctx: ToolContext) {
  ...
  let chatResults: ReturnType<typeof searchChatMessages> = [];
...
export const searchDataTool: Tool = {
  ...
  input_schema: {
    ...
    properties: {
      ...
      source: { type: 'string', description: '数据源过滤', enum: ['feishu', 'gitlab', 'github', 'linear', 'jira', 'posthog', 'figma', 'notion', 'confluence', 'slack', 'discord', 'google_drive', 'google_calendar', 'email'] },
  ...
  async execute(input: Record<string, unknown>, ctx: ToolContext): Promise<string> {
    ...
    if (accessible.length > 0) {
      ...
      for (const obj of accessible) {
        ...
        parts.push(
          `- [${obj.source}/${obj.source_type}]${hasContent} ${obj.title}${obj.summary ? ' — ' + obj.summary.slice(0, 200) : ''} (URI: ${obj.uri}, 更新: ${obj.updated_at?.slice(0, 10) ?? '未知'})`
        );
      }
    }
    ...
    if (parts.length === 0) return '未找到匹配的数据对象或群聊消息。';
    return parts.join('\n');
  },

  async executeStructured(input: Record<string, unknown>, ctx: ToolContext): Promise<{ text: string; structured: StructuredResult }> {
    const result = await doSearch(input, ctx);
    if (!result) return { text: 'ERROR: 用户不存在', structured: { type: 'list', items: [], total: 0 } };
    const { accessible, chatResults, query } = result;

    // 结构化 items
    const items: StructuredListItem[] = accessible.map(obj => ({
      title: obj.title,
