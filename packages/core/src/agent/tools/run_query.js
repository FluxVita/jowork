...
export const runQueryTool = {
    ...
    input_schema: {
        type: 'object',
        properties: {
            source: { type: 'string', description: '数据源', enum: ['feishu', 'gitlab', 'linear', 'posthog', 'figma', 'email'] },
    ...
    async execute(input, ctx) {
        ...
        if (accessible.length === 0)
            return '未找到符合条件的数据对象。';
        return `共 ${accessible.length} 条结果：\n` + accessible.map(obj => `- [${obj.source}/${obj.source_type}] ${obj.title} (URI: ${obj.uri}, 敏感级别: ${obj.sensitivity}, 更新: ${obj.updated_at?.slice(0, 10) ?? '未知'})`).join('\n');
    },
    async executeStructured(input, ctx) {
        const result = doQuery(input, ctx);
        if (!result)
            return { text: 'ERROR: 用户不存在', structured: { type: 'table', columns: [], rows: [], total: 0 } };
        const { accessible } = result;
...
//# sourceMappingURL=run_query.js.map