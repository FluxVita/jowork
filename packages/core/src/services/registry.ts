...
function rowToService(row: ServiceRow): Service {
  return {
    ...
    default_roles: JSON.parse(row.default_roles_json || '[]') as Role[],
...
export function registerService(svc: {
  ...
  `).run(
    ...
    svc.status ?? 'active',
    svc.icon ?? null,
    JSON.stringify(svc.default_roles ?? []),
    svc.requires_config ? 1 : 0,
    svc.sort_order ?? 0,
    svc.data_scope ?? 'public',
  );
...
export function getService(id: string): Service | null {
  const db = getDb();
  const row = db.prepare('SELECT * FROM services WHERE service_id = ?').get(id) as ServiceRow | undefined;
  return row ? rowToService(row) : null;
}

/** 列出服务（支持过滤） */
export function listServices(opts?: { type?: ServiceType; status?: ServiceStatus; data_scope?: DataScope }): Service[] {
  const db = getDb();
...
/** 更新服务字段 */
export function updateService(id: string, fields: Partial<{
  name: string;
  description: string;
  category: string;
  endpoint: string;
  ...
  if (fields.config !== undefined) { sets.push('config_json = ?'); params.push(JSON.stringify(fields.config)); }
  if (fields.icon !== undefined) { sets.push('icon = ?'); params.push(fields.icon); }
  if (fields.default_roles !== undefined) { sets.push('default_roles_json = ?'); params.push(JSON.stringify(fields.default_roles)); }
  if (fields.requires_config !== undefined) { sets.push('requires_config = ?'); params.push(fields.requires_config ? 1 : 0); }
  if (fields.sort_order !== undefined) { sets.push('sort_order = ?'); params.push(fields.sort_order); }
  if (fields.data_scope !== undefined) { sets.push('data_scope = ?'); params.push(fields.data_scope); }
...
/** 更新服务状态 */
export function updateServiceStatus(id: string, status: ServiceStatus): void {
  const db = getDb();
