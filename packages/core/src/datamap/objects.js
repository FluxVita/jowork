...
export function upsertObject(obj) {
  ...
  `).run(id, obj.source, obj.source_type, obj.uri, obj.external_url ?? null, obj.title, obj.summary ?? null, obj.sensitivity, JSON.stringify(obj.acl), JSON.stringify(obj.tags), obj.etag ?? null, obj.owner ?? null, obj.content_type ?? null, obj.size_bytes ?? null, obj.updated_at, now, obj.ttl_seconds, obj.connector_id, obj.metadata ? JSON.stringify(obj.metadata) : null, obj.data_scope ?? 'public', obj.content_path ?? null, obj.content_length ?? null);
    // 同步更新 FTS5 索引
    syncObjectFts(db, id, obj.title, obj.summary, obj.content_path);
    return id;
}
/** 同步 objects_fts 索引（upsert 时调用） */
function syncObjectFts(db, objectId, title, summary, contentPath) {
    // 读取全文内容（如果有 content_path）
    const content = contentPath ? (readContentByPath(contentPath) ?? '') : '';
    // 先查 rowid
    const existing = db.prepare('SELECT rowid FROM objects WHERE object_id = ?').get(objectId);
    if (!existing)
        return;
...
export function searchObjects(opts) {
    ...
    const rows = db.prepare(`SELECT * FROM objects ${where} ORDER BY updated_at DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
    return rows.map(rowToObject);
}
/** FTS5 全文搜索 — 同时搜 objects_fts，按相关度排序 */
function searchWithFts(db, opts, limit, offset) {
    ...
    try {
      ...
      ORDER BY rank
      LIMIT ? OFFSET ?
    `).all(ftsExpr, ...params, limit, offset);
        if (rows.length > 0)
            return rows.map(rowToObject);
    }
    catch {
    ...
    if (opts.source) {
        likeConditions.push('o.source = ?');
        likeParams.push(opts.source);
    }
    if (opts.source_type) {
        likeConditions.push('o.source_type = ?');
        likeParams.push(opts.source_type);
    }
    if (opts.sensitivity) {
        likeConditions.push('o.sensitivity = ?');
        likeParams.push(opts.sensitivity);
    }
    if (opts.data_scope) {
        likeConditions.push('o.data_scope = ?');
        likeParams.push(opts.data_scope);
    }
    if (opts.tags?.length) {
        for (const tag of opts.tags) {
    ...
    const rows = db.prepare(`SELECT o.* FROM objects o WHERE ${likeConditions.join(' AND ')} ORDER BY o.updated_at DESC LIMIT ? OFFSET ?`).all(...likeParams, limit, offset);
    return rows.map(rowToObject);
}
...
export function searchChatMessages(opts) {
    ...
    // 空数组 → 用户不在任何群，直接返回空
    if (opts.allowed_chat_ids && opts.allowed_chat_ids.length === 0)
        return [];
    const words = opts.query.trim().split(/\s+/).filter(Boolean);
    if (words.length === 0)
        return [];
...
export function getObject(objectId) {
    const db = getDb();
    const row = db.prepare('SELECT * FROM objects WHERE object_id = ?').get(objectId);
    return row ? rowToObject(row) : null;
}
// ─── 通过 URI 获取 ───
export function getObjectByUri(uri) {
    const db = getDb();
    const row = db.prepare('SELECT * FROM objects WHERE uri = ?').get(uri);
    return row ? rowToObject(row) : null;
}
// ─── 统计 ───
export function getStats() {
    ...
    const bySource = db.prepare('SELECT source, COUNT(*) as n FROM objects GROUP BY source').all();
    const bySensitivity = db.prepare('SELECT sensitivity, COUNT(*) as n FROM objects GROUP BY sensitivity').all();
