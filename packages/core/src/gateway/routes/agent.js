...
import { listMcpServers, addMcpServer, removeMcpServer, getAllMcpToolDefs, executeMcpTool, setMcpServerActive, updateMcpServer, getMcpServerStatus, getOrCreateBridge, getActiveMcpServers } from '../../agent/mcp-bridge.js';
...
router.post('/chat', authMiddleware, requireFeishuAuth, async (req, res) => {
    const { message, session_id, engine, images } = req.body;
    ...
    try {
        ...
        // 外部工具执行器
        const externalToolExecutor = async (name, input) => {
...
router.post('/feedback', authMiddleware, (req, res) => {
    const { message_id, session_id, rating, comment } = req.body;
...
export function registerFileToken(path, filename) {
