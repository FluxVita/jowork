...
const ALERT_THRESHOLDS = [
    { ratio: 0.70, level: 'warning' },
    { ratio: 0.85, level: 'degraded' },
    { ratio: 0.95, level: 'critical' },
];
/** 记录一次 API 调用 */
export function trackApiCall(source, category, count = 1) {
    const db = getDb();
...
/** 检查是否允许调用（配额保护） */
export function canCallFeishu(category) {
