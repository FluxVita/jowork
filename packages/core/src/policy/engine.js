...
export function checkAccess(user, object, action) {
    // owner 总是 admin 权限
    if (user.role === 'owner') {
    ...
    const db = getDb();
    const policies = db.prepare('SELECT * FROM policies WHERE is_active = 1 ORDER BY priority DESC').all();
    for (const policy of policies) {
        const roles = JSON.parse(policy.roles_json);
        const actions = JSON.parse(policy.actions_json);
        // 角色匹配
        if (!roles.includes(user.role))
            continue;
        // 敏感级别匹配（null = 匹配所有级别）
        if (policy.sensitivity && policy.sensitivity !== object.sensitivity)
            continue;
        // 操作匹配
...
/** 检查对象级 ACL */
function checkACL(user, acl, action) {
    ...
    const matchesRef = (r) => r === userRef || r === roleRef || r === allRef ||
...
export function filterByAccess(user, objects) {
...
/** 检查是否允许本地下沉 */
export function canDownload(sensitivity) {
