 ...
 */
export function getSensitivityCeiling(opts: ContextPepOpts): Sensitivity {
...
/** 检查某敏感级别是否在天花板以内 */
export function isWithinCeiling(sensitivity: Sensitivity, ceiling: Sensitivity): boolean {
...
/** 可能包含敏感信息的正则模式 */
const SENSITIVE_PATTERNS: Array<{ pattern: RegExp; label: string; replacement: string }> = [
  // 金额（薪资、融资等）
...
): { content: string; redacted: boolean; redactedLabels: string[] } {
 ...
 */
export function getSecurityPromptSegment(opts: ContextPepOpts): string {
  const ceiling = getSensitivityCeiling(opts);
