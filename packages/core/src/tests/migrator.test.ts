...
describe('migrate — fresh installation', () => {
  test('applies all migrations on a brand-new DB', async () => {
    ...
    assert.deepEqual(result.applied, ['001_initial', '002_messages_fts', '003_connector_items', '004_connector_sync_schedule', '005_message_feedback', '006_session_pinned_folder']);
  });
...
describe('migrate — bootstrap for existing DB', () => {
  test('marks 001_initial as applied if users table already exists', async () => {
    ...
    assert.deepEqual(result.applied, ['002_messages_fts', '003_connector_items', '004_connector_sync_schedule', '005_message_feedback', '006_session_pinned_folder']);
